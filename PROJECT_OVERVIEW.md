...
- **Productionize the CropRow lab.** The beta crop-row localizer already runs live and on uploaded video with unique plant counting; the next steps are persistent ByteTrack IDs (already in the training notebooks), the ≥30 FPS end-to-end target, and per-row yield/gap analytics for cultivator guidance.
